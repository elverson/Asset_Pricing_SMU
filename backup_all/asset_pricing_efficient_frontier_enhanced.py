...
Industry_Portfolios.xlsx contains monthly nominal (net) returns for ten industry portfolios, expressed in percent. 
...
with expected (monthly) return on the vertical axis and standard deviation of (monthly) return on the 
horizontal axis. This plot should cover the range from 0% to 2% on the vertical axis, in increments of 0.1% (or less).
...
→ Plot the efficient frontier (with the riskless asset) on the same plot as 
the minimum-variance frontier generated by the ten industry portfolios.
...
→ Calculate the Sharpe ratio for the tangency portfolio, and also the tangency portfolio 
weights for the ten industry portfolios.
...
Minimum-variance frontier consists of all portfolios with least risk for 
specified mean return ⇒ no portfolios exist to the left of this frontier.

Efficient frontier consists of all portfolios with highest mean return for 
specified amount of risk ⇒ risk-averse investor must invest in portfolio on this frontier to maximise expected utility.
